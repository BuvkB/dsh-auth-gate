...
export function parseListen(authority: string): { hostname: string; port: number } {
  const at = authority.lastIndexOf(":");
  if (at === -1) return { hostname: authority, port: 8443 };
 ...
 */
export function assertLoopbackListen(authority: string): { hostname: string; port: number } {
  const { hostname, port } = parseListen(authority);
  if (!isLoopbackHostname(hostname))
    throw new Error(
      `proxy: --listen host ${JSON.stringify(hostname)} is not loopback; bind 127.0.0.1 only`,
    );
