...
> 范围：新增 dsh-auth-gate 的 client 半边（browser 插件），在 dsh GUI 侧边栏底部
> 挂一个登出按钮。服务端端点（`POST /auth/logout`、`GET /auth/status`）M2/M3 已
> 冻结并发布（0.5.1），本规格只动渲染层与打包层。
...
- **挂载点**：`sidebar.footer.action` —— list 槽位、root 级恒显示、`replaceRisk:
none`（可追加，不替换任何 shipped UI）。注册契约 `{ id, order, label }`；owner
  props 为 `{ wide: boolean }`（false = 56px rail 模式）。
...
| #   | 决策                                                                                                                                                                                                                                                                                      |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1  | 挂载点 `sidebar.footer.action`，entry id `dsh-auth-gate-logout`，`order: 0`，`label: "Sign out"`（i18n 延后，暂英文）                                                                                                                                                                     |
...
| D4  | 视觉：`wide=true` 图标+文字；`wide=false` 仅图标、label 视觉隐藏但保留可访问名（仿 settings.trigger 契约）。内联 SVG（logout 图标，`stroke="currentColor"`），样式用内联 style，不引 CSS 文件、不引 primitives（依赖最小化）                                                              |
...
| D7  | 测试：jsdom（`@vitest-environment jsdom` pragma）单测覆盖组件分支（authenticated 真/假、wide 真/假、form action/method、可访问名）与 apply 注册参数；fetch mock                                                                                                                           |
...
| `src/client/index.tsx`                   | 新       | client 插件：`export const inject = ["slots"]`；`apply(ctx)` 内 `ctx.slots.inject("sidebar.footer.action", () => ctx.slots.register({...}, LogoutAction))`                                                                                    |
| `src/client/logout-action.tsx`           | 新       | `LogoutAction({ wide })`：useEffect fetch `/auth/status` → 门控渲染；`<form method="post" action="/auth/logout?next=/">` + 内联 SVG + 可访问名                                                                                                |
...
4. 部署到生产（tencent-cloud，`dsh-web.service`）后真实浏览器验证：登录 →
   侧边栏底部出现 Sign out → 点击 → 302 回落登录页 → 未带 cookie 的 SPA 请求被
