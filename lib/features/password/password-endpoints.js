...
<<<<<<< HEAD:lib/features/password/password-endpoints.js
import { buildSetCookie } from "../../session/index.js";
=======
import { buildSetCookie } from "./session-store.js";
/** 从 Accept-Language 头判断登录页语言（zh 开头 => 中文）。 */
function langOf(req) {
    try {
        const h = req.headers?.["accept-language"] ?? "";
        if (/^\s*zh/i.test(h) || /,\s*zh/i.test(h))
            return "zh";
    }
    catch (_e) {
        // ignore
...
>>>>>>> a02ec5d (feat(login): 登录页全面改版 —— Apple 风极简设计，保留 DeepSeek 品牌与标语动效):lib/password-endpoints.js
/**
