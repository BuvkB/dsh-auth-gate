import { validateNext, parseFormBody, parseCookieHeader, langOf, totpChallengePageHtml, } from "../../shared/index.js";
...
export async function handlePasswordLogin(deps, req, res) {
    ...
    if (challenge !== undefined && code !== "" && deps.totpMode !== "off") {
        await handleTotpSubmit(deps, res, challenge, code, next, ip, lang);
        return;
    }
    await handlePasswordSubmit(deps, res, params, next, ip);
}
/** TOTP 挑战提交：限速 → 用户文件 → 恒时验证 → 防重放 → 禁用检查 → 发会话。 */
async function handleTotpSubmit(deps, res, username, code, next, ip, lang) {
...
function rejectTotp(deps, res, next, lang) {
    deps.logger.info("login rejected");
    res.setHeader("cache-control", "no-store");
    res.writeHead(401, { "content-type": "text/html; charset=utf-8" });
    res.end(totpChallengePageHtml(next, "invalid credentials", lang));
}
