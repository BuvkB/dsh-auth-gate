 ...
 */
import { CARD_STYLE, SHIELD_SVG, EYE_OPEN_SVG, EYE_CLOSED_SVG, USER_ICON_SVG, LOCK_ICON_SVG, EYE_SCRIPT, CURSOR_SCRIPT, } from "./login-assets.js";
/** 登录页文案字典：按浏览器语言返回中文或英文（默认英文）。 */
function loginStrings(lang) {
    const zh = lang === "zh";
    return {
        htmlLang: zh ? "zh" : "en",
 ...
 */
function buildSloganHtml(brandText) {
 ...
 */
export function loginPageHtml(next, error, lang) {
...
export function passwordLoginPageHtml(next, error, lang) {
    const s = loginStrings(lang);
    return renderLoginCard({
