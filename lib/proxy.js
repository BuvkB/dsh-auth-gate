...
import { assertLoopbackListen, bearerOf, filterRequestHeaders, filterResponseHeaders, } from "./proxy-headers.js";
...
function forwardUpgrade(options, req, clientSocket, clientHead, log) {
    ...
    upstream.on("upgrade", (up, upSocket, upHead) => {
        ...
        for (const [name, value] of Object.entries(filterResponseHeaders(up.headers, options.stripSecureCookie))) {
            if (value === undefined)
                continue;
