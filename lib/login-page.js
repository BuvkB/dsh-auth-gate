...
const CARD_STYLE = `
  * { box-sizing: border-box; }
  ...
  .brand { width: 48px; height: 48px; border-radius: 14px; background: linear-gradient(135deg, #4d6bfe, #7c5cff); display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; }
  ...
  input { width: 100%; height: 44px; padding: 0 14px; border: 1px solid #d9dce1; border-radius: 10px; font-size: 14px; color: #1f2329; background: #fff; outline: none; transition: border-color .15s, box-shadow .15s; }
  ...
  button[type=submit] { width: 100%; height: 44px; border: 0; border-radius: 10px; background: #4d6bfe; color: #fff; font-size: 15px; font-weight: 600; cursor: pointer; margin-top: 4px; transition: background .15s, transform .05s; }
  button[type=submit]:hover { background: #4059e0; }
  button[type=submit]:active { transform: scale(.99); }
  .error { color: #dc2626; font-size: 13px; margin: 0 0 16px; text-align: center; }
...
`;
const SHIELD_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="m9 12 2 2 4-4"/></svg>';
...
function renderLoginCard(options) {
    const errorHtml = options.error === undefined ? "" : `<p class="error">${escapeHtml(options.error)}</p>`;
